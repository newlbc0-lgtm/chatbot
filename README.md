...
> **최종 상태**: ✅ 구글 스프레드시트 연동 + Claude AI + UptimeRobot 24시간 무중단 자동 깨우기 100% 구축 완료
...
```text
  ...
  [ Render FastAPI 서버 ] ─────┼─────▶ [ 🧠 Claude AI (LLM) ]
       ▲                       │
       │ (5분마다 자동 핑)       ▼ (손님 대화 발송)
  [ ⏰ UptimeRobot ]     [ 카카오톡 & 네이버 톡톡 손님 대화창 ]
```
...
| 🌐 **무로그인 웹 위젯 스크립트** | `https://chatbot-9g4i.onrender.com/widget.js` | 홈페이지 부착용 자바스크립트 위젯 (로그인 0%) |
| 🖥️ **웹 위젯 실시간 데모** | [https://chatbot-9g4i.onrender.com/widget-demo](https://chatbot-9g4i.onrender.com/widget-demo) | 홈페이지 챗봇 미리보기 및 체험 |
| ⏰ **UptimeRobot 모니터링** | [https://uptimerobot.com](https://uptimerobot.com) | **24시간 서버 무중단 자동 깨우기 (5분 주기)** |
...
* **`google_sheets.py`** ([google_sheets.py](file:///d:/python-prg/chatbot/google_sheets.py)): 
  * 사장님의 `chatbot` 구글 시트 데이터를 실시간 직접 API 연결하여 읽어오는 핵심 연동 모듈 (60초 초고속 캐싱)
...
## 4. ⏰ 24시간 365일 무중단 서버 유지 (UptimeRobot)

* **상태**: ✅ 활성화 완료
* **모니터링 대상**: `https://chatbot-9g4i.onrender.com`
* **작동 주기**: 5분 간격 핑 발송
* **효과**: Render 무료 클라우드 서버의 슬립 모드(수면 상태) 및 `502 BAD_GATEWAY` 타임아웃 에러 100% 방지
